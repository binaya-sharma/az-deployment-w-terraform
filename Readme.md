...
The first version intentionally uses **one Azure Databricks workspace and its serverless default-storage catalog**. Terraform manages the development schemas and grants inside that catalog without adding customer-managed ADLS. The bundle and workflow already expose `dev`, `qual`, and `prod` targets; the future [multi-workspace DAB design](docs/deployment/dab-multi-environment-notes.md) maps them to separately governed workspaces without copying application definitions.
